...
class DataSet(ISTAT):
        ...
        def set_identifiers(self, dataflow_identifier):
            ...
            if dataflow_identifier[3] == '_':
                ...
                if type(dataflow_identifier) == str: return self.set_from_description(dataflow_identifier)
                else: raise ValueError(dataflow_identifier)

        def set_from_id(self, df_id):
            mask = self.available['df_id'] == df_id
            df = self.available[mask]
            return df.to_dict(orient='records')[0]

        def set_from_structure_id(self, df_structure_id):
            mask = self.available['df_structure_id'] == df_structure_id
            df = self.available[mask]
            return df.to_dict(orient='records')[0]

        def set_from_description(self, description):
            mask = self.available['description'] == description
            df = self.available[mask]
            return df.to_dict(orient='records')[0]
        ...
        def get_dimensions(self, dataframe = True):
            """Return a Dataframe containing the dimensions and their description descriptions of a specific dataflow."""
        ...
        def dimension_values(self, dimension_ID, dataframe = True):
            """Return ALL the possible values of a dimension in the dataset"""
            #TODO: only allow dimensions present in the dataset
            resource = "codelist"
            path_parts = [resource, self.agencyID, dimension_ID]
            path = "/".join(path_parts)
            response = self._request(path = path)
